...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`fixed top-0 left-0 z-50 h-full w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:z-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        {/* Logo */}
        <div className="flex items-center gap-3 px-5 py-5 border-b border-gray-200 dark:border-gray-800">
          ...
          <div>
            <h1 className="text-base font-bold text-gray-900 dark:text-white">Youtube Manage</h1>
            <p className="text-xs text-gray-500 dark:text-gray-400">Auto Content Generator</p>
          </div>
        </div>

        {/* Navigation */}
        <nav className="px-3 py-4 space-y-0.5 overflow-y-auto h-[calc(100%-140px)]">
          {menuItems.map((item) => {
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 border-t border-gray-200 dark:border-gray-800">
          <div className="card p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-xs font-medium text-gray-900 dark:text-white">System Active</p>
                <p className="text-[10px] text-gray-500 dark:text-gray-400">All APIs connected</p>
              </div>
            </div>
          </div>
        </div>
