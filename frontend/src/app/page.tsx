...
export default function Dashboard() {
  ...
  const allConnected = Object.values(health).every(v => v === 'connected')
  const apiStatus = allConnected ? 'All Connected' : 'Setup Required'

  const stats = [
    {
      title: 'Total Videos',
      value: String(statsData.total_videos),
      change: `${statsData.generating} generating`,
      icon: (
    ...
    {
      title: 'Uploaded',
      value: String(statsData.uploaded),
      change: 'to YouTube',
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {
      title: 'Scheduled',
      value: String(statsData.scheduled),
      change: 'in queue',
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {
      title: 'API Status',
      value: allConnected ? 'Ready' : 'Setup',
      change: apiStatus,
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 card p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <a href="/library" className="text-sm text-primary-600 dark:text-primary-400 hover:underline font-medium">
              View all
            </a>
          </div>
          {statsData.recent_videos.length > 0 ? (
            <div className="space-y-3">
              {statsData.recent_videos.map((video: any) => (
                <div key={video.id} className="flex items-center justify-between p-4 rounded-lg bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-gray-900 dark:text-white text-sm">{video.title || video.keyword}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">{video.niche} • {video.created_at}</p>
                    </div>
                  </div>
                  <span className={`text-xs font-medium px-2.5 py-1 rounded-full capitalize ${statusColors[video.status] || ''}`}>
                    {video.status}
                  </span>
                </div>
              ))}
